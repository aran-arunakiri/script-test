...
var summary_days = {}  # Reused map for DailySummary response

def update_eco_state()
  if !eco_mode_enabled
    eco_state = "off"
  elif total_battery_capacity != nil
    eco_state = "active"
  else
    eco_state = "pending"
  end
...
def init_weekday_totals()
  ...
  for day_name: weekday_names
    weekday_totals[day_name] = {
      "total_energy": 0.0,
      "session_count": 0,
      "eco_sessions": 0
    }
...
def start_calibration(battery_percentage)
  ...
  if energy
    if battery_percentage != nil && battery_percentage >= 0 && battery_percentage < 100
      ...
      log("Error: Battery percentage must be between 0-99%", 2)
      return {"error": "Battery percentage must be between 0-99%"}
    end
  else
...
def monitor_energy()
  if eco_state == "active" && is_charging
...
tasmota.add_cmd("StartCal", def(cmd, idx, payload, payload_json)
  ...
  if percentage != nil
    ...
    tasmota.resp_cmnd({"StartCal": {"error": "Please specify battery percentage (0-99)"}})
  end
end)
...
tasmota.add_cmd("EcoMode", def(cmd, idx, payload, payload_json)
  if payload
    if payload == "1" || string.tolower(payload) == "on" || string.tolower(payload) == "true"
      ...
      update_eco_state()
      save_data()
      tasmota.resp_cmnd({"EcoMode": {"status": "ON", "eco_state": eco_state}})
    elif payload == "0" || string.tolower(payload) == "off" || string.tolower(payload) == "false"
      eco_mode_enabled = false
      update_eco_state()
      save_data()
      tasmota.resp_cmnd({"EcoMode": {"status": "OFF", "eco_state": eco_state}})
    end
  else
    tasmota.resp_cmnd({"EcoMode": {"status": eco_mode_enabled ? "ON" : "OFF", "eco_state": eco_state}})
  end
end)
...
tasmota.add_cmd("DailySummary", def(cmd, idx, payload, payload_json)
  ...
  for i: 0..charging_history.size()-1
    ...
    if session["eco_mode_active"]
      summary_days[date]["eco_sessions"] += 1
    end
